...
{
    public partial class Buddhabrot : Form
    {
        ...
        {
            int imgWidth = pctBx_Display.Height, imgHeight = pctBx_Display.Width;
            ...
            int[,,] redbuffer = new int[workers, imgWidth, imgHeight];
            int[,,] greenbuffer = new int[workers, imgWidth, imgHeight];
            int[,,] bluebuffer = new int[workers, imgWidth, imgHeight];

            int redIterations = 50;
            int greenIterations = 500;
            int blueIterations = 5000;

            int maxIteration = Math.Max(redIterations, Math.Max(greenIterations, blueIterations));
            int numValues = 5000000 / workers;
            double valueRadius = 3;
            ...
            Parallel.For(0, workers, index => {
                ...
                {
                    iteration = 0;
                    c = Complex.FromPolarCoordinates(val.NextDouble() * valueRadius, val.NextDouble() * 2d * Math.PI);
                    z = new Complex(0, 0);
                    valueBuffer = new Complex[maxIteration];

                    while (iteration < maxIteration)
                    {
                        z = z * z + c;
                        valueBuffer[iteration] = z;
                        ...
                        if ((z.Real * z.Real + z.Imaginary * z.Imaginary) >= 4d)
                        {
                            ...
                            {
                                xInd = (int)((valueBuffer[v].Real - firstPixelGCoord.X) / adjPlotScale);
                                yInd = (int)((valueBuffer[v].Imaginary - firstPixelGCoord.Y) / adjPlotScale);
                                if (xInd >= 0 && xInd < imgWidth && yInd >= 0 && yInd < imgHeight)
                                {
                                    if (v < redIterations)
                                        redbuffer[index, xInd, yInd]++;
                                    if (v < greenIterations)
                                        greenbuffer[index, xInd, yInd]++;
                                    if (v < blueIterations)
                                        bluebuffer[index, xInd, yInd]++;
                                }
                            }
                            break;
                        }
            ...
            pctBx_Display.Image = new Bitmap(imgHeight, imgWidth);
            ...
            for (int i = 0; i < imgWidth; i++)
            {
                for (int j = 0; j < imgHeight; j++)
                {
                    ...
                    {
                        red += redbuffer[w, i, j];
                        green += greenbuffer[w, i, j];
                        blue += bluebuffer[w, i, j];
                    }
                    ((Bitmap)pctBx_Display.Image).SetPixel(j, i, Color.FromArgb(red % 255, green % 255, blue % 255));
                }
            }
        ...
        private Tuple<Complex, Complex, Complex> JuliaPixel(double x, double y, int redIterations, int greenIterations, int blueIterations, int maxIterations)
        {
            int iteration = 0;
            Complex c = new Complex(x, y);
            Complex z = new Complex(0, 0);

            Complex redEscapeValue = new Complex(0, 0), greenEscapeValue = new Complex(0, 0), blueEscapeValue = new Complex(0, 0);
            ...
            while (iteration < maxIterations)
            {
                ...
                if ((z.Real * z.Real + z.Imaginary * z.Imaginary) >= 4d)
                {
                    if (iteration < redIterations)
                        redEscapeValue = z;

                    if (iteration < greenIterations)
                        greenEscapeValue = z;

                    if (iteration < blueIterations)
                        blueEscapeValue = z;
                    break;
                }
            ...
            return new Tuple<Complex, Complex, Complex>(redEscapeValue, greenEscapeValue, blueEscapeValue);
        }
    ...
    public class PaletteManager : Buddhabrot
    {
